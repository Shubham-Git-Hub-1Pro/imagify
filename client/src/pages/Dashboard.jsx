...
import api from '../axios'; // use the axios instance with baseURL + Authorization interceptor
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        console.log('Fetching dashboard data — using api instance with token:', !!token);

        // Use api (baseURL from VITE_API_URL or fallback) and interceptor will add Authorization header.
        const [creditsResponse, generationsResponse] = await Promise.all([
          api.get('/api/user/credits'),
          api.get('/api/image/user-generations')
        ]);

        console.log('Credits response:', creditsResponse?.data);
        console.log('Generations response:', generationsResponse?.data);

        const creditsOK = creditsResponse?.data?.success;
        const gensOK = generationsResponse?.data?.success;
        ...
        if (creditsOK && gensOK) {
          ...
          const errMsg = creditsResponse?.data?.message || generationsResponse?.data?.message || 'Failed to fetch dashboard data';
          console.error('Dashboard data error:', errMsg);
          setError(errMsg);
          if (errMsg.toLowerCase().includes('auth') || errMsg.toLowerCase().includes('login')) {
            navigate('/');
          }
          toast.error(errMsg);
        }
        ...
        setError(errMsg);
        if (err?.response?.status === 401 || errMsg.toLowerCase().includes('auth') || errMsg.toLowerCase().includes('login')) {
          navigate('/');
        }
        toast.error(errMsg);
      } finally {
  ...
  }, [token, navigate]); // backendUrl removed: we use api instance
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gradient-to-r from-teal-500 to-orange-500 rounded-xl shadow-sm p-8 text-white mb-8 relative overflow-hidden">
          ...
          <h1 className="text-3xl font-bold mb-4 relative">
            ...
            <motion.span className="inline-block cursor-pointer relative text-white font-bold">
          ...
          <motion.p className="text-lg opacity-90 mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            Your creative journey continues...
          </motion.p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <motion.div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm" whileHover={{ scale: 1.02 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <button onClick={handleGenerateClick} className="bg-teal-500 text-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all hover:scale-105">
            <div className="text-2xl mb-2">🎨</div>
            <h3 className="font-semibold">New Creation</h3>
            <p className="text-sm opacity-90 mt-1">Start generating</p>
          </button>
          <Link to="/gallery" className="bg-orange-500 text-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all hover:scale-105">
            <div className="text-2xl mb-2">🖼️</div>
            <h3 className="font-semibold">Gallery</h3>
            <p className="text-sm opacity-90 mt-1">View your work</p>
          </Link>
          <Link to="/buy" className="bg-purple-500 text-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all hover:scale-105">
            <div className="text-2xl mb-2">⭐</div>
            <h3 className="font-semibold">Get Credits</h3>
            <p className="text-sm opacity-90 mt-1">Power up</p>
          </Link>
          <Link to="/features" className="bg-blue-500 text-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all hover:scale-105">
            <div className="text-2xl mb-2">✨</div>
        ...
        {/* Rest of UI (tips & resources) - unchanged */}
        <div className="bg-gradient-to-br from-white to-teal-50 rounded-xl shadow-sm p-6 border border-gray-100 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <button onClick={handleGenerateClick} className="text-teal-600 hover:text-teal-700 text-sm font-medium">Try Now →</button>
            </div>
            <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <button onClick={handleGenerateClick} className="text-orange-500 hover:text-orange-600 text-sm font-medium">Try Now →</button>
            </div>
            <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <button onClick={handleGenerateClick} className="text-purple-600 hover:text-purple-700 text-sm font-medium">Try Now →</button>
            </div>
          </div>
        </div>
